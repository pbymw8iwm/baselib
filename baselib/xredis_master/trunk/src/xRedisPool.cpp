...
bool RedisPool::ConnectRedisDB( unsigned int cahcetype,  unsigned int dbindex,
	...
	unsigned int poolsize, unsigned int timeout, unsigned int role,int databaseIndex){
    if( (NULL==host)
    ...
    return mRedisCacheList[cahcetype].ConnectRedisDB(cahcetype, dbindex, host, port,
		passwd, poolsize, timeout, role, databaseIndex);
}
...
bool RedisDBSlice::ConnectRedisNodes(unsigned int cahcetype, unsigned int dbindex, const std::string& host, unsigned int port, const std::string& passwd, unsigned int poolsize, unsigned int timeout, int role, int databaseIndex)
{
    bool bRet = false;
    if ((host.empty())
        || (cahcetype > MAX_REDIS_CACHE_TYPE)
        ...
        if (MASTER == role) {
            ...
            for (unsigned int i = 0; i < poolsize; ++i) {
                ...
                if (pRedisconn->RedisConnect()) {
					...
					char strCmd[1024];
					sprintf(strCmd,"select %d",databaseIndex);
					redisReply *reply = static_cast<redisReply *>(redisCommand(pRedisconn->getCtx(), strCmd)); 
			...
			if (mSliceConn.RedisMasterConn.size() != 0){
				bRet = true;
			}            
        } else if (SLAVE == role) {
...
bool RedisCache::ConnectRedisDB(unsigned int cahcetype, unsigned int dbindex, const char *host, 
	unsigned int port, const char *passwd, unsigned int poolsize, unsigned int timeout, unsigned int role, int databaseIndex)
{
