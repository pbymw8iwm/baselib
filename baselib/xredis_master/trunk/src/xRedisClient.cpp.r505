...
bool xRedisClient::ConnectRedisCache(const RedisNode *redisnodelist, unsigned int hashbase, unsigned int cachetype, int databaseIndex) {
    if (NULL==mRedisPool) {
        return false;
    }
    ...
    for (unsigned int n = 0; n<hashbase; n++) {
        ...
        bool bRet = mRedisPool->ConnectRedisDB(cachetype, pNode->dbindex, pNode->host.c_str(), pNode->port, 
			pNode->passwd.c_str(), pNode->poolsize, pNode->timeout, pNode->role, databaseIndex);
        if (!bRet) {
            return false;
        }
    }
