...
typedef struct _REDIS_NODE_{
	...
	string       host;          //  REDIS节点主机IP地址
	unsigned int port;          //  redis服务端口
	string       passwd;        //  redis认证密码
...
public:
    ...
    bool ConnectRedisCache( const RedisNode *redisnodelist, unsigned int hashbase, unsigned int cachetype,int databaseIndex);
