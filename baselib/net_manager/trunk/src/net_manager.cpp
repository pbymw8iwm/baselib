...
Net_Manager::Net_Manager()
{
	...
	m_send_packet_queue = new Net_Send_Packet_Queue;
}

Net_Manager::~Net_Manager()
{
	delete m_net_event_queue;
	delete m_send_packet_queue;
}
...
uint32_t Net_Manager::create_udp(const INET_Addr& local_addr, void* pUserData, int netbufsize)
{
	...
	SOCK_Dgram *dgram = new SOCK_Dgram(this, pUserData);
	...
	{
		delete pNetPacket;
		LOG(WARN)("Net_Manager::create_udp error, send task queue is full");
		return 0;
	}
...
uint32_t Net_Manager::create_tcp_client(const INET_Addr& remote_addr, Packet_Splitter *packet_splitter, void* pUserData, int timeout, int netbufsize)
{
	if (1 != m_status) {
	...
	SOCK_Connector *connector = new SOCK_Connector(this, packet_splitter, pUserData);
	...
	{
		delete pNetPacket;
		LOG(WARN)("Net_Manager::create_tcp_client error, send task queue is full");
		return 0;
	}
...
uint32_t Net_Manager::create_tcp_client(const char *remote_ip, int remote_port, Packet_Splitter *packet_splitter, 
										void* pUserData, int timeout, int netbufsize)
{
	INET_Addr remote_addr;
	remote_addr.set_addr(inet_addr(remote_ip));
	remote_addr.set_port(htons(remote_port));

	return create_tcp_client(remote_addr, packet_splitter, pUserData, timeout, netbufsize);
}

uint32_t Net_Manager::create_tcp_server(const INET_Addr& local_addr, Packet_Splitter *packet_splitter, 
										void* pUserData, int netbufsize)
{
	if (1 != m_status) {
	...
	SOCK_Acceptor *acceptor = new SOCK_Acceptor(this, packet_splitter, pUserData);
	...
	{
		delete pNetPacket;
		LOG(WARN)("Net_Manager::create_tcpserver error, send task queue is full");
		return 0;
	}
										...
										void* pUserData, int netbufsize)
{
	...
	return create_tcp_server(local_addr, packet_splitter, pUserData, netbufsize);
}
...
int Net_Manager::delete_net(uint32_t id)
{
	...
	{
		delete pNetPacket;
		LOG(WARN)("Net_Manager::delete_net error, send task queue is full");
		return 0;
	}
